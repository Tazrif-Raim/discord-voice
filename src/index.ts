  ...
  try {
    ...
    try {
      const geminiSession = await getGeminiSession(interaction.guildId!, {
        systemInstruction:
          'You are a helpful and friendly AI voice companion in a Discord voice channel. Your name is "Bini". Keep responses concise and conversational.',
      });
      geminiSessions.set(interaction.guildId!, geminiSession);
      conversationStates.set(interaction.guildId!, "IDLE"); // Initialize state
      console.log(
        `🧠 Gemini session established for guild ${interaction.guildId}`
      );
    } catch (error) {
...
function setupVoiceReceiver(
  ...
  receiver.speaking.on("start", async (userId) => {
    ...
    console.log(
      `🎤 Speaking start event - user: ${userId}, state: ${currentState}`
    );

    // Only accept new speech in IDLE state
    if (currentState !== "IDLE") {
      console.log(
        `⏳ Ignoring speech - not in IDLE state (current: ${currentState})`
      );
      return;
    }
